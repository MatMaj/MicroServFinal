$(document).ready(function(){
    ...
    function showMicro() {
       ...
       $.ajax({                                                                        /* Ajax call to AccountMicroservice */
           url: 'http://localhost:8383/showMicro',
           type: "POST",
           success: function (data, status, xhr) {
              console.log(data);
              $("#bodyTableMicro").empty();
              jQuery.each(data.response, function(i, val) {
                 $("#bodyTableMicro").append("<tr><th>" + val.id + "</th><td>" + val.description + "</td><td>" + val.priority + "</td><td>" + convertDate(val.date) + "</td><td><p class='delete' onclick='deleteMicro(" + val.id + ")'></p></td></tr>");
              });
           },
    ...
    function deleteMicro(id) {
       ...
       $.ajax({
          url: 'http://localhost:8383/deleteMicro/' + id,
          type: "POST",
          success: function (data, status, xhr) {
             console.log(data);
    ...
    $("#newMicro").click(function(e) {
       e.preventDefault();
       if( $("#newMicroPriority").val() == null ){
           alert("Insert a valid priority value");
       } else if ($("#newMicroDescription").val() == ""){
           ...
           $.ajax({                                                        /* Ajax call to AccountMicroservice for login */
              ...
              type: "POST",
              data: {
                 description: $("#newMicroDescription").val(),
                 priority: $("#newMicroPriority").val(),
                 fkUser: $("#newMicroEmail").val()
              },
              success: function (data, status, xhr) {
                 emptyForm();
                 showMicro();
    ...
    function emptyForm() {
       $("#newMicroDescription").val("");
       $("#newMicroPriority").val('');
    }